...
require_once __DIR__ . '/../../../ai_analytics/includes/layout.php';
    ...
    if (empty($disciplineName)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                ...
                ON DUPLICATE KEY UPDATE file_path=VALUES(file_path), text_content=VALUES(text_content), status='processed'
            ");
            $stmt->execute([
                ':disc' => $disciplineName,
                ':course' => $courseNumber,
            ...
            $syllabusId = (int)($pdo->lastInsertId() ?: $pdo->query("SELECT id FROM user_syllabuses WHERE discipline_name=".
                $pdo->quote($disciplineName)." AND course_number=".(int)$courseNumber." AND file_name=".$pdo->quote($file['name'])." AND uploaded_by=1")->fetchColumn());
            ...
            $stmtCount = $pdo->prepare("SELECT COUNT(*) FROM ai_syllabus_topics WHERE discipline_name=:disc AND course_number=:course");
            $stmtCount->execute([':disc' => $disciplineName, ':course' => $courseNumber]);
            $savedTopics = (int)$stmtCount->fetchColumn();
            ...
            header('Location: ../import.php?import_tab=syllabus');
            exit;
